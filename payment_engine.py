...
PAYMENT_MODE = os.getenv("PAYMENT_MODE", "test")  # "test" or "live"
...
class PaymentEngine:
    ...
    def create_razorpay_order(self, amount_inr: int, receipt: str, notes: Dict = None) -> Dict:
        ...
        amount_inr: Amount in paise (e.g., 9900 = ₹99)
        """
        ...
        # Live mode — would call Razorpay API
        try:
            import requests
    ...
    def create_paypal_order(self, amount_usd: float, description: str) -> Dict:
        """Create a PayPal order for international payments."""
    ...
    def verify_paypal_payment(self, order_id: str, paypal_order_id: str) -> Dict:
        ...
        # Live verification would call PayPal API
        try:
            # Get OAuth token
            import requests
    ...
    def verify_upi_payment(self, txn_id: str, amount: float, notes: str = "") -> Dict:
        """Record UPI payment verification."""
    ...
    def _save_payment(self, data: Dict):
        ...
        logger.info(f"Payment recorded: {data.get('order_id')} - {data.get('method')} - {data.get('amount')}")
    ...
    def _update_payment(self, order_id: str, status: str, txn_id: str = None):
        ...
        for payment in self.payments:
            if payment.get("order_id") == order_id:
                ...
                logger.info(f"Payment {order_id} → {status}")
                return
